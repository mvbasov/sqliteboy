    ...
    'default':
        {
            ...
            'h_interpreter': 'hint: monkey programming language interpreter',
...
def proc_nosb(handle):
    ...
    if isnosb():
        ...
        if path.startswith('/login') or  \
            ...
            path.startswith('/interpreter') or \
...
$if user():
    ...
    <a href='/interpreter'>$_['cmd_interpreter']</a>
...
$if data['command'] in crt.keys():
    ...
    $elif data['command'] == 'calculator':
        ...
        $else:
            $rt()
    $elif data['command'] == 'interpreter':
        $ crtq = data['message']
        $if crtq:
            $crtq[crt['interpreter']]
        $else:
            $rt()
$else:
    $rt()
...
class notes:
    def GET(self):
        ...
        xaction = [
                    ...
                    (_['cmd_run'], '/interpreter?src=notes&sid='),
                ]
        if isadmin():
            xaction.append(
...
class interpreter:
    def GET(self):
        ...
        #
        data = {
            'title': '%s' %(_['tt_interpreter']),
            'command': 'interpreter',
            'action_url': '/interpreter',
            'action_method': 'post',
            'action_button': (
                                ('run', _['cmd_run'], False, '', 'submit'),
                            ),
            'hint': _['h_interpreter'],
            'message': smsgq(SK_INTERPRETER),
            'interpreter': q,
        }
        content = ''
        stop()
    ...
    def POST(self):
        q = web.input(q='').q.strip()
        if not q:
            raise web.seeother('/interpreter')
        #
        error = 1
        #
        start()
        d = {
                'SQLITEBOY_NAME': NAME,
                'SQLITEBOY_VERSION_STRING': VERSION,
                'SQLITEBOY_VERSION_INTEGER': int(VERSION.replace('.','')),
            }
        try:
            e = MonkeyEnvironment.from_dictionary(d)
        ...
        t = rt()
        #
        sess[SK_INTERPRETER] = [q, err, msg, t, error]
        raise web.seeother('/interpreter')
